...
const resolveBackends = (config: GrprConfig, rootDir: string): ResolvedBackends => {
  const errors: BackendError[] = [];
  const backends: BackendEntry[] = [];
  const storeDir = resolveStoreDir(config, rootDir);
  const readConfig = config.read ?? { backend: "local" };

  const addBackend = (backendConfig: GrprReadBackendConfig): void => {
...
const sortEventsDesc = (events: GrprEvent[]): GrprEvent[] => {
...
): Promise<{ events: GrprEvent[]; truncated: boolean; errors?: BackendError[] }> => {
...
export const readTail = async (
  config: GrprConfig,
  rootDir: string,
  params: GrprTailParams & { since?: string },
): Promise<{ events: GrprEvent[]; truncated: boolean; errors?: BackendError[] }> => {
