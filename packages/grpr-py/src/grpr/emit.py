...
from .schema import GrprEvent, GrprLevel
from .store import append_event

_cached: Optional[Dict[str, Any]] = None

_DEFAULT_RUN_ID = os.environ.get("GRPR_RUN_ID") or str(uuid.uuid4())
_DEFAULT_SESSION_ID = os.environ.get("GRPR_SESSION_ID")
...
def _normalize_level(level: Optional[str]) -> GrprLevel:
    if not level:
        return "info"
    lower = level.lower()
...
def _to_event(input_event: Dict[str, Any], defaults: Dict[str, str]) -> GrprEvent:
    event: GrprEvent = {
